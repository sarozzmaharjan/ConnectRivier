...
function user_verify(username, thislink, unfollow_linkid, msg_after)
{
	...
	req.open("POST", siteurl+"ajax/verify/r:"+Math.round(Math.random()*1000), true);
	req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
var dbrd_tabs_to_check	= ["all", "@me", "private", "commented", "feeds", "tweets","notifications"];
	...
	req.onreadystatechange	= function() {
		...
		if( txt.length > 0 ) {
			...
			for(i=0; i<txt.length; i++) {
				...
				if( tb!="all" && tb!="@me" && tb!="private" && tb!="notifications" && tb!="commented" && tb!="feeds" && tb!="tweets" ) {
					continue;
				}
