...
function postform_set_hotkeys() {
	...
	var f_onkeypress	= function(e) {
		...
		if( pf_hotkeys_enabled && (code==126 || code==1087) && !e.ctrlKey && !e.altKey && pf_open_state==0 && !flybox_opened ) {
...
function postform_open_step2(load_data)
{
	...
	pf_data	= ({
		...
		at_counter_for: ["image", "file", "videoembed", "link"]	//number of attached images
	});
	...
	d.getElementById("att_ok_image_container").style.display	= "none";
	d.getElementById("att_ok_image_container").innerHTML		= ""; 
	d.getElementById("att_ok_file_container").style.display	= "none";
	d.getElementById("att_ok_file_container").innerHTML		= "";
	d.getElementById("att_ok_link_container").style.display	= "none";
	d.getElementById("att_ok_link_container").innerHTML		= "";
	d.getElementById("att_ok_videoembed_container").style.display	= "none";
	d.getElementById("att_ok_videoembed_container").innerHTML	= "";
	
	pf_data.at_counter_for["image"] = 0; 
	pf_data.at_counter_for["file"] = 0; 
	pf_data.at_counter_for["videoembed"] = 0;
	pf_data.at_counter_for["link"] = 0; 
...
function postform_open_step2_loadpost()
{
	...
	req.onreadystatechange	= function() {
		...
		for(i=0; i<data.length; i++) {
			...
			pf_data.attachments++;
			pf_data.at_counter_for[tp]++;

			pf_data["at_"+tp][pf_data.at_counter_for[tp]-1]	= [id, txt];
			create_new_attachment(tp, pf_data.at_counter_for[tp], txt);
		}
		postform_open_step3();
	}
	req.open("POST", siteurl+"ajax/postform-loadpost/ajaxtp:xml/r:"+Math.round(Math.random()*1000), true);
	req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	req.send("postid="+encodeURIComponent(pf_data.existing_post_id)+"&tmp_id="+encodeURIComponent(pf_data.temp_id));
}
...
function create_new_attachment(type, number, attext)
{
	var appendWhere = document.getElementById('att_ok_'+type+'_container');
	
	if(appendWhere.style.display=='none') appendWhere.style.display='block';
	
	var div = document.createElement('div'); 
	div.setAttribute('id','attachok_'+type+'_'+number);
	div.setAttribute('class','attachok');
	...
	b.innerHTML = d.getElementById("att_ok_"+type+"_container").getAttribute("title");
	span.appendChild(b);
	
	var em = document.createElement('em');
	em.setAttribute('id','attachok_'+type+'_txt_'+number);
	span.appendChild(em);
	...
	a.setAttribute('class','removeattachment');
	a.setAttribute('onclick',"postform_attach_remove('"+type+"', "+number+");");
	a.setAttribute('onfocus','this.blur();');
	span.appendChild(a);
	
	appendWhere.appendChild(div);
}
function delete_attachment(type, number)
{
	var deleteWhere = document.getElementById('att_ok_'+type+'_container');
	var deleteWhat = document.getElementById('attachok_'+type+'_'+number);
	deleteWhere.removeChild(deleteWhat);
}
...
function postform_attach_submit(callback_if_ok)
{
	...
	if( pf_attach_state_tp == "image" ) {
		if( d.post_form["atch_image_"+pf_data.atchimgtab].value == "" ) { //pf_data.atchimgtab - upl or url 
			d.post_form["atch_image_"+pf_data.atchimgtab].focus();
			return;
		}
		d.post_form["atch_image_"+pf_data.atchimgtab].disabled	= true;	
		pf_data.at_counter_for["image"] ++; 
	}
	else {
		...
		pf_data.at_counter_for[pf_attach_state_tp]++;
	}
	...
	var f_ok	= function(attxt) {
		if( ! pf_data["at_"+pf_attach_state_tp][pf_data.at_counter_for[pf_attach_state_tp]-1] ) { 
			pf_data.attachments	++;
		}

		pf_data["at_"+pf_attach_state_tp][pf_data.at_counter_for[pf_attach_state_tp]-1]	= [-1];

		d.getElementById("attachbtn_"+pf_attach_state_tp).style.display	= "block";
		create_new_attachment(pf_attach_state_tp, pf_data.at_counter_for[pf_attach_state_tp], false);
		
		if(d.getElementById("att_ok_"+pf_attach_state_tp+"_container").style.display	== "none"){
			d.getElementById("att_ok_"+pf_attach_state_tp+"_container").style.display	= "block";
		}
		
		d.getElementById("attachok_"+pf_attach_state_tp+"_"+pf_data.at_counter_for[pf_attach_state_tp]).style.display	= "block";
		d.getElementById("attachok_"+pf_attach_state_tp+"_txt_"+pf_data.at_counter_for[pf_attach_state_tp]).innerHTML	= attxt;
		var tmpf	= function() {
			pf_attach_state	= 0;
			if( pf_attach_state_tp == "image" ) {
	...
	var post_id = (pf_data.existing_post_id != "")? pf_data.existing_post_id:"";
	...
	switch( pf_attach_state_tp ) {
		...
		case "videoembed":
			...
			req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			req.send("post_temp_id="+encodeURIComponent(pf_data.temp_id)+"&at_type="+pf_attach_state_tp+"&post_id="+encodeURIComponent(post_id)+"&data="+encodeURIComponent(d.post_form["atch_"+pf_attach_state_tp].value));
			break;
		case "file":
			...
			var f_check	= function() {
				...
				req.open("POST", siteurl+"ajax/postform-attach/ajaxtp:xml/r:"+Math.round(Math.random()*1000), true);
				req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
				req.send("post_temp_id="+encodeURIComponent(pf_data.temp_id)+"&at_type=file&data="+encodeURIComponent(frmkey)+"&post_id="+encodeURIComponent(post_id));
			};
		...
		case "image":
			...
			var f_check	= function() {
				...
				req.open("POST", siteurl+"ajax/postform-attach/ajaxtp:xml/r:"+Math.round(Math.random()*1000), true);
				req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
				req.send("post_temp_id="+encodeURIComponent(pf_data.temp_id)+"&at_type=image&data="+encodeURIComponent(dt)+"&post_id="+encodeURIComponent(post_id));
			}
			setTimeout(f_check, 1);
			break;
		default:
...
function postform_attach_remove(at_type, at_number)
{ 
	if( pf_open_state!=1 || pf_data.attachments==0 || !pf_data["at_"+at_type][at_number-1] ) {
		return;
	}
	...
	pf_data.attachments	--;
	pf_data.at_counter_for[at_type]--; 
	
	pf_data["at_"+at_type][at_number-1]	= []; 
	if( at_type == "image" ) {
		d.post_form["atch_image_"+pf_data.atchimgtab].value	= "";
	}
	else {
	...
	d.getElementById("attachok_"+at_type+"_"+at_number).style.display	= "none"; 
	
	delete_attachment(at_type, at_number);
	...
	req.open("POST", siteurl+"ajax/postform-attach-remove/ajaxtp:txt/r:"+Math.round(Math.random()*1000), true);
	req.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	req.send("post_temp_id="+encodeURIComponent(pf_data.temp_id)+"&at_type="+encodeURIComponent(at_type)+"&at_id="+encodeURIComponent(at_number-1)+"&post_id="+encodeURIComponent(post_id));
}
...
function postform_submit_step4()
{
	...
	d.getElementById("att_ok_image_container").style.display	= "none"; //added
	d.getElementById("att_ok_file_container").style.display	= "none"; //added
	d.getElementById("att_ok_videoembed_container").style.display	= "none"; //added
	d.getElementById("att_ok_link_container").style.display	= "none"; //added
